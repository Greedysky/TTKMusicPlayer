...
void MusicBackgroundRemoteWidget::buttonClicked(int index)
{
    ...
    QDir dir( "." );
    dir.mkpath( QString("%1%2").arg(MusicObject::getAppDir() + THEME_CACHED).arg(m_currentIndex) );
...
void MusicBackgroundRemoteWidget::itemUserClicked(QListWidgetItem *item)
{
    ...
    {
        emit showCustomSkin(QString("%1%2/%3%4").arg(MusicObject::getAppDir() + THEME_CACHED)
...
QStringList MusicBackgroundRemoteWidget::createPaths()
{
    ...
    for(int i=0; i<count; ++i)
    {
        paths << QString("%1%2/%3%4").arg(MusicObject::getAppDir() + THEME_CACHED)
