...
static void readfile(const unsigned char *inptr, const int inlen)
{
  ...
  if (d-inptr < inlen)
    ...
    printf("after read: est %d, is %d\n",inlen,d-inptr);
    printf("midisize: %d, globs: %d, patches: %d\n",base.midisize,base.globsize,base.patchsize);
}
...
static sInt transEnv(sInt enVal)
{
  ...
  if (nEnVal<0)
  {
    printf("!!clamping enval lower bound!\n");
    nEnVal=0;
  }
  else if (nEnVal>127)
  {
    printf("!!clamping enval upper bound!\n");
    nEnVal=127;
  }
...
void ConvertV2M(const unsigned char *inptr, const int inlen, unsigned char **outptr, int *outlen)
{
    ...
    if (maxp2!=base.maxp)
    {
        printf("warning: patch count inconsistency: we:%d, they:%d\n",base.maxp,maxp2);
        base.maxp=maxp2;
    }
