...
MusicBDQueryArtistRequest::MusicBDQueryArtistRequest(QObject *parent)
    : MusicQueryArtistRequest(parent)
{
    m_queryServer = QUERY_BD_INTERFACE;
}

void MusicBDQueryArtistRequest::startToSearch(const QString &artist)
{
    if(!m_manager)
    {
        return;
...
void MusicBDQueryArtistRequest::downLoadFinished()
{
...
void MusicBDQueryArtistRequest::getDownLoadIntro(MusicResultsItem *item)
{
    if(!m_manager)
    {
        return;
