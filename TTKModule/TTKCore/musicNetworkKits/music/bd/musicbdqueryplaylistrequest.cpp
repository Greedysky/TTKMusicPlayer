...
MusicBDQueryPlaylistRequest::MusicBDQueryPlaylistRequest(QObject *parent)
...
void MusicBDQueryPlaylistRequest::startToSearch(QueryType type, const QString &playlist)
{
    if(type == MusicQuery)
    {
...
void MusicBDQueryPlaylistRequest::startToPage(int offset)
{
    if(!m_manager)
    {
        return;
...
void MusicBDQueryPlaylistRequest::startToSearch(const QString &playlist)
{
    if(!m_manager)
    {
        return;
    ...
    const QUrl &musicUrl = MusicUtils::Algorithm::mdII(BD_PLAYLIST_INFO_URL, false).arg(playlist);
    m_interrupt = true;
...
void MusicBDQueryPlaylistRequest::getPlaylistInfo(MusicResultsItem &item)
{
    if(!m_manager)
    {
        return;
    ...
    const QUrl &musicUrl = MusicUtils::Algorithm::mdII(BD_PLAYLIST_INFO_URL, false).arg(item.m_id);
...
void MusicBDQueryPlaylistRequest::downLoadFinished()
{
    if(!m_reply)
    {
        deleteAll();
...
void MusicBDQueryPlaylistRequest::getDetailsFinished()
{
