...
MusicBDDiscoverListRequest::MusicBDDiscoverListRequest(QObject *parent)
    : MusicDiscoverListRequest(parent)
{
    m_pageSize = 100;
}

void MusicBDDiscoverListRequest::startToSearch()
{
    if(!m_manager)
    {
        return;
...
void MusicBDDiscoverListRequest::downLoadFinished()
{
    if(!m_reply)
    {
        deleteAll();
