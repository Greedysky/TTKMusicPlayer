...
MusicBDQueryAlbumRequest::MusicBDQueryAlbumRequest(QObject *parent)
    : MusicQueryAlbumRequest(parent)
{
    m_queryServer = QUERY_BD_INTERFACE;
}

void MusicBDQueryAlbumRequest::startToSearch(const QString &album)
{
    if(!m_manager)
    {
        return;
...
void MusicBDQueryAlbumRequest::startToSingleSearch(const QString &artist)
{
    if(!m_manager)
    {
        return;
    ...
    const QUrl &musicUrl = MusicUtils::Algorithm::mdII(BD_ARTIST_ALBUM_URL, false).arg(artist);
    m_interrupt = true;
...
void MusicBDQueryAlbumRequest::downLoadFinished()
{
...
void MusicBDQueryAlbumRequest::singleDownLoadFinished()
{
