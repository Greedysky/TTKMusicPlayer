...
MusicMGDiscoverListRequest::MusicMGDiscoverListRequest(QObject *parent)
    : MusicDiscoverListRequest(parent)
{

}

void MusicMGDiscoverListRequest::startToSearch()
{
    if(!m_manager)
    {
        return;
    ...
    request.setUrl(MusicUtils::Algorithm::mdII(MG_SONG_LIST_URL, false).arg(23603703).arg(0).arg(20));
...
void MusicMGDiscoverListRequest::downLoadFinished()
{
        ...
        if(ok)
        {
            ...
            if(value.contains("result"))
            {
                ...
                for(const QVariant &var : qAsConst(datas))
                {
                    ...
                    m_toplistInfo = QString("%1 - %2").arg(singerName.first().toString()).arg(value["songName"].toString());
                    break;
                }
            }
        }
