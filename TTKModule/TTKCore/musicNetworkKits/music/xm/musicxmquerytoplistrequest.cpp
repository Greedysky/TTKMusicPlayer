...
MusicXMQueryToplistRequest::MusicXMQueryToplistRequest(QObject *parent)
    : MusicQueryToplistRequest(parent)
{
    m_queryServer = QUERY_XM_INTERFACE;
}

void MusicXMQueryToplistRequest::startToSearch(QueryType type, const QString &toplist)
{
    if(type == MusicQuery)
    {
...
void MusicXMQueryToplistRequest::startToSearch(const QString &toplist)
{
    if(!m_manager)
    {
        return;
