...
MusicXMQueryArtistRequest::MusicXMQueryArtistRequest(QObject *parent)
    : MusicQueryArtistRequest(parent)
{
    m_queryServer = QUERY_XM_INTERFACE;
}

void MusicXMQueryArtistRequest::startToSearch(const QString &artist)
{
    if(!m_manager)
    {
        return;
...
void MusicXMQueryArtistRequest::downLoadFinished()
{
...
void MusicXMQueryArtistRequest::getDownLoadIntro(MusicResultsItem *item)
{
    if(!m_manager)
    {
        return;
    ...
    makeTokenQueryUrl(&request, true,
                      MusicUtils::Algorithm::mdII(XM_ARTIST_INFO_DATA_URL, false).arg(m_searchText),
