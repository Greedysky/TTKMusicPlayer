        ...
        if(ok)
        {
            ...
            foreach(const QVariant &value, datas)
            {
                ...
                if(m_counter < 5 && !dataMap.isEmpty())
                {
                    ...
                    M_LOGGER_ERROR(url);
                    MusicDataDownloadThread *download = new MusicDataDownloadThread(url, QString("%1%2%3%4").arg(BACKGROUND_DIR_FULL).arg(m_savePath).arg(m_counter++).arg(SKN_FILE), MusicObject::DownloadBigBG, this);
