...
void MusicSoundKMicroWidget::positionChanged(qint64 position)
{
    ...
    m_ui->timeLabel->setText(QString("%1/%2").arg(TTKTime::msecTime2LabelJustified(position * MT_S2MS), TTKTime::msecTime2LabelJustified(m_ui->timeSlider->maximum())));
...
void MusicSoundKMicroWidget::durationChanged(qint64 duration)
{
    ...
    m_ui->timeLabel->setText(QString("%1/%2").arg(MUSIC_TIME_INIT, TTKTime::msecTime2LabelJustified(duration * MT_S2MS)));
...
void MusicSoundKMicroWidget::playFinished()
{
    ...
    if(m_ui->gifLabel->isRunning())
    {
        ...
        const QString &path = MusicUtils::File::getSaveFileName(this, "Wav(*.wav)");
        if(!path.isEmpty())
        {
