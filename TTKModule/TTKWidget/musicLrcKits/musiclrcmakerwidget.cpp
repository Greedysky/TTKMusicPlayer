...
{
    m_plainText.clear();
    m_analysis->setFilePath(QString("%1%2%3").arg(TTK::String::lrcDirPrefix(), name, LRC_FILE));
...
void MusicLrcMakerWidget::positionChanged(qint64 position)
{
    ...
    if(m_ui->stackedWidget->currentIndex() == 3)
    {
        ...
        {
            if(currentLrc != m_lrcContainer[m_analysis->middle()]->text())
            {
...
{
    MusicApplication::instance()->playTimePosition(value);
    m_analysis->findTimePosition(value);
}
...
void MusicLrcMakerWidget::reMakeButtonClicked()
{
    ...
    if(message.exec())
    {
        MusicApplication::instance()->playTimePosition(0);
...
void MusicLrcMakerWidget::thirdWidgetStateButtonClicked()
{
    ...
    if(state)
    {
        m_lrcContainer[m_analysis->middle()]->start(m_intervalTime);
    }
    else
    {
        m_lrcContainer[m_analysis->middle()]->stop();
        m_ui->lrcViewer->stop();
    }
}
...
void MusicLrcMakerWidget::setCurrentThirdWidget()
{
    ...
    if(m_times.count() == m_plainText.count())
    {
        ...
        m_analysis->loadFromBuffer(data);
...
{
    m_analysis->setTimePosition(TTK_DN_S2MS);
    updateCurrentLrc(m_analysis->findTimePosition(m_ui->timeSlider_T->value()));
}

void MusicLrcMakerWidget::lrcPositionFaster()
{
    m_analysis->setTimePosition(-TTK_DN_S2MS);
    updateCurrentLrc(m_analysis->findTimePosition(m_ui->timeSlider_T->value()));
}
...
void MusicLrcMakerWidget::createCurrentLine(int key)
{
    ...
    if(m_ui->stackedWidget->currentIndex() == 2 && m_plainText.count() > m_currentLine)
    {
        switch(key)
        {
            case Qt::Key_Left:
            {
                MusicApplication::instance()->playTimePosition(m_ui->timeSlider_F->value() - 2222);
                m_lineItem->moveLeft();
                break;
            }
            case Qt::Key_Up:
...
void MusicLrcMakerWidget::createThirdWidget()
{
    ...
    connect(m_ui->lrc_make_up_T, SIGNAL(clicked()), SLOT(lrcPositionSlower()));
    connect(m_ui->lrc_make_down_T, SIGNAL(clicked()), SLOT(lrcPositionFaster()));
