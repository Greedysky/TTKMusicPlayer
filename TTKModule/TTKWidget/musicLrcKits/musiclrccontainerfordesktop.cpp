...
void MusicLrcContainerForDesktop::applyParameter()
{
    ...
    m_singleLine = !(G_SETTING_PTR->value(MusicSettingManager::DLrcSingleLineMode).toInt() == 1);
...
{
    m_singleLine = !m_singleLine;
    G_SETTING_PTR->setValue(MusicSettingManager::DLrcSingleLineMode, m_singleLine);
...
void MusicLrcContainerForDesktop::mousePressEvent(QMouseEvent *event)
{
    ...
    {
        m_offset = QtGlobalPosition(event) - frameGeometry().topLeft();
    }
}
...
void MusicLrcContainerForDesktop::mouseMoveEvent(QMouseEvent *event)
{
    ...
    if(!m_windowLocked && (event->buttons() & Qt::LeftButton))
    {
        ...
        move(QtGlobalPosition(event) - m_offset);
...
void MusicLrcContainerForDesktop::contextMenuEvent(QContextMenuEvent *event)
{
    ...
    menu.addAction(!m_singleLine ? tr("Single Line") : tr("Double Line"), this, SLOT(setSingleLineTypeChanged()));
    menu.addSeparator();
