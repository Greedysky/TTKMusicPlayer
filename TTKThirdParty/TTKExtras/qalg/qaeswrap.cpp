...
QByteArray QAesWrap::encrypt(const QByteArray &in, const QByteArray &key, const QByteArray &iv)
{
    ...
    const int nNumber = (nLen % 16 > 0) ? nTotal - nLen :16;
    ...
    AES_cbc_encrypt((unsigned char *)enc_s, (unsigned char *)encrypt_string, nTotal, &aes, (unsigned char *)iv.data(), AES_ENCRYPT);
    delete[] enc_s;
...
QByteArray QAesWrap::decrypt(const QByteArray &in, const QByteArray &key, const QByteArray &iv)
{
    ...
    AES_cbc_encrypt((unsigned char *)decode, (unsigned char *)encrypt_string, strlen(decode), &aes, (unsigned char *)iv.data(), AES_DECRYPT);
