...
MusicDownLoadQueryArtistVipThread::MusicDownLoadQueryArtistVipThread(QObject *parent)
    : MusicDownLoadQueryThreadAbstract(parent)
{

}

QString MusicDownLoadQueryArtistVipThread::getClassName()
{
    return staticMetaObject.className();
}

void MusicDownLoadQueryArtistVipThread::startSearchSong(QueryType type, const QString &text)
{
    Q_UNUSED(type);
...
void MusicDownLoadQueryArtistVipThread::downLoadFinished()
{
    if(m_reply == nullptr)
    {
        deleteAll();
...
QString MusicDownLoadQueryArtistVipThread::getCurrentURL() const
{
    int index = M_SETTING_PTR->value(MusicSettingManager::DownloadServerChoiced).toInt();
    switch( index )
    {
        case 0:  return MusicCryptographicHash::decryptData(MUSIC_ARTIST_MULTI_VIP_WY, URL_KEY);
        case 2:  return MusicCryptographicHash::decryptData(MUSIC_ARTIST_MULTI_VIP_QQ, URL_KEY);
        case 3:  return MusicCryptographicHash::decryptData(MUSIC_ARTIST_MULTI_VIP_XM, URL_KEY);
        case 4:  return MusicCryptographicHash::decryptData(MUSIC_ARTIST_MULTI_VIP_TT, URL_KEY);
        case 5:  return MusicCryptographicHash::decryptData(MUSIC_ARTIST_MULTI_VIP_BD, URL_KEY);
        case 6:  return MusicCryptographicHash::decryptData(MUSIC_ARTIST_MULTI_VIP_KW, URL_KEY);
        case 7:  return MusicCryptographicHash::decryptData(MUSIC_ARTIST_MULTI_VIP_KG, URL_KEY);
    }
    return QString();
}

void MusicDownLoadQueryArtistVipThread::readFromMusicSongAttribute(MusicObject::MusicSongInfomation &info,
