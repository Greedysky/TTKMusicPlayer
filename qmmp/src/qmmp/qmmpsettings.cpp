    ...
    : QObject(parent)
{
    ...
    m_aud_format = static_cast<Qmmp::AudioFormat>(settings.value("Output/format", Qmmp::PCM_S16).toInt());
    m_aud_dithering = settings.value("Output/dithering", true).toBool();
    m_volume_step = settings.value("Output/volume_step", 10).toInt();
...
void QmmpSettings::setNetworkSettings(bool use_proxy, bool auth, ProxyType type, const QUrl &proxy)
{
    ...
    m_proxy_url = proxy;
    if(type == HTTP_PROXY)
        m_proxy_url.setScheme("http");
    else if(type == SOCKS5_PROXY)
        m_proxy_url.setScheme("socks5");
    saveSettings();
