...
{
public:
    Magic(std::size_t offset, const char *magic)
        : m_offset(offset), m_magic(magic),
          m_len(strlen(magic))
    {

    }

    Magic(std::size_t offset, const char *magic, std::size_t len)
        : m_offset(offset),
          m_magic(magic),
          m_len(len)
    {
    ...
    virtual bool matches(InputStream &stream) override
    {
...
private:
    static quint16 le16(const unsigned char *p)
    {
        return ((quint16)( p[1]) <<  8) | ( p[0]);
    }

    static quint32 le32(const unsigned char *p)
    {
        return((quint32)le16(&p[2]) << 16) | le16(&p[0]);
    }
