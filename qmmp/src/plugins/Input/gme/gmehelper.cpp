...
Music_Emu *GmeHelper::load(const QString &url, int sample_rate)
{
    ...
    if((err = gme_identify_file(Qmmp::generatePrintable(path),&file_type)))
    {
    ...
    if((err = gme_load_file(m_emu, Qmmp::generatePrintable(path))))
    {
        qWarning("GmeHelper: %s", err);
        return nullptr;
    }
    ...
    m3u_path.append(".m3u");
    gme_load_m3u(m_emu, Qmmp::generatePrintable(m3u_path));
    m_path = path;
    return m_emu;
}
